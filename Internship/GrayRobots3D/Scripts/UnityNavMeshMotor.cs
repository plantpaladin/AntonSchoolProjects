...
public class UnityNavMeshMotor : RAINMotor
{
    ...
    public override bool IsFacing (MoveLookTarget aTarget)
	{
		...
		if(Vector3.Dot((AI.Kinematic.Position-aTarget.Position).normalized, AI.Body.transform.forward)>=CloseEnoughAngle)
			return true;
	...
	public override bool IsFacing (Vector3 aPosition)
	{
		...
		if(Vector3.Dot((AI.Kinematic.Position-aPosition).normalized, AI.Body.transform.forward)>=CloseEnoughAngle)
			return true;
		return false;
	}
